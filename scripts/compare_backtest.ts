...
async function runCompoundBacktest(symbol: string, csvFile: string, slBuffer: number = 0.003) {
  ...
  console.log(`\n${'='.repeat(50)}`);
  console.log(`  ${symbol} - COMPOUND BACKTEST (2021-2022)`);
...
async function main() {
  await runCompoundBacktest('BTC/USDT', 'btc_5m_history.csv', 0.003);
  await runCompoundBacktest('ETH/USDT', 'eth_5m_history.csv', 0.005);
}
