...
async function runCompoundBacktest(symbol: string, timeframe: string, slBuffer: number) {
  const csvFile = `btc_${timeframe}_history.csv`;
...
async function main() {
  await runCompoundBacktest('BTC/USDT', '15m', 0.003);
  await runCompoundBacktest('BTC/USDT', '30m', 0.004);
  await runCompoundBacktest('BTC/USDT', '4h', 0.008);
}
