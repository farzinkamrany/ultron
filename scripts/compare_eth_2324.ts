...
async function runCompoundBacktest2324(slBuffer: number) {
  const csvFile = `eth_30m_2324.csv`;
  ...
  const candles: any[] = [];
  let currentYear = 0;
  let year2023Balance = 0;
  let year2024Balance = 0;
  let totalTrades = 0;
  let wins = 0;
  let maxDrawdown = 0;
  ...
  for await (const line of rl) {
    ...
    if (currentYear !== 0 && currentYear !== candleYear && currentYear === 2023) {
      year2023Balance = balance;
    }
  ...
  year2024Balance = balance;

  console.log(`\n${'='.repeat(50)}`);
  console.log(`  ETH/USDT - 30m TIMEFRAME COMPOUND BACKTEST (2023-2024)`);
  console.log(`${'='.repeat(50)}`);
  console.log(`  سرمایه اولیه:          $${INITIAL_CAPITAL}`);
  console.log(`  پایان سال اول (2023):   $${year2023Balance.toFixed(2)}`);
  console.log(`  پایان سال دوم (2024):   $${year2024Balance.toFixed(2)}`);
...
runCompoundBacktest2324(0.008).catch(console.error);
