...
const START_TIMESTAMP = Number(process.env.TEST_START); const END_TIMESTAMP = Number(process.env.TEST_END);
...
async function runMultiAssetOrchestrator() {
    ...
    for (const sym of SYMBOLS) {
        try {
            ...
            for (const c of data) {
                if (c.timestamp >= START_TIMESTAMP && c.timestamp <= END_TIMESTAMP) globalTimeline.push(c);
            }
    ...
    let globalBalance = INITIAL_CAPITAL; let totalVolumeUSD = 0;
            ...
            for (const level of state.grid) {
                ...
                if (level.type === 'BUY' && candle.low <= level.price) {
                    ...
                    if (coinsBought > 0.0001) {
                        ...
                        state.realizedGridPnl += actualCost * Math.abs(MAKER_FEE); totalVolumeUSD += actualCost;
                    ...
                    if (state.positionCoins > 0) {
                        ...
                        if (coinsSold > 0.0001) {
                            const actualValue = coinsSold * level.price; totalVolumeUSD += actualValue;
