          ...
          {messages.map((msg: Message) => (
            ...
            >
              ...
              <div className={`max-w-[80%] p-4 rounded-2xl text-sm leading-relaxed ${msg.role === \"user\"
                ...
                {msg.role === \"assistant\" && (
                  <button onClick={() => speak(msg.content)} className=\"mt-3 p-1.5 rounded-full hover:bg-white/10 text-phase1\">
      ...
      <div className=\"p-4 border-t border-white/10 bg-black/40\">
        <form onSubmit={sendMessage} className=\"relative flex items-end gap-2\">
          ...
          <TextareaAutosize
            ...
            className=\"flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-slate-200 placeholder:text-slate-500 focus:outline-none transition-all resize-none\"
          />
          
          <button
            type=\"submit\"
            disabled={!input.trim() || isLoading}
            className=\"p-3 rounded-xl bg-phase1 hover:bg-phase1/80 text-slate-900 font-bold transition-all disabled:opacity-50 shrink-0\"
          >
...
export default function UltronDashboard() {
  ...
  const { 
    ...
    addMessage, setInput, setIsLoading, setIsVoiceActive, updateLastMessage
  } = useChatStore();
  ...
  return (
    <div className=\"min-h-screen bg-slate-950 text-slate-200 p-4 md:p-8 font-sans\">
        ...
        <header className=\"mb-8 flex justify-between items-center\">
            <div>
                <h1 className=\"text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-phase1 to-phase2\">ULTRON</h1>
                <p className=\"text-[10px] text-slate-400 tracking-[0.3em] uppercase\">Tactical Command Center</p>
            </div>
        </header>
        <main className=\"grid grid-cols-1 lg:grid-cols-12 gap-6\">
            ...
            <div className=\"lg:col-span-4 flex flex-col gap-6\">
                <DashboardModule title=\"LIVE MARKET\" icon={Activity} variant=\"phase1\">
